<?php include ("../template/menu.php");?>
      <!-- /nav -->
      <!-- page wrapper starts -->
      <div id="page-wrapper">
         <!-- Jumbotron -->
         <div class="jumbotron jumbotron-fluid">
            <div class="container" >
               <!-- jumbo-heading -->
               <div class="jumbo-heading" data-aos="fade-down">
                  <h1>Cursos</h1>
                  <!-- Breadcrumbs -->
                  <nav aria-label="breadcrumb">
                     <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a>Home</a></li>
                        <li class="breadcrumb-item"><a>Cursos</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Português 5º Ano</li>
                     </ol>
                  </nav>
                  <!-- /breadcrumb -->
               </div>
               <!-- /jumbo-heading -->
            </div>
            <!-- /container -->
         </div>
         <!-- /jumbotron -->
         <!-- ==== Page Content ==== -->
         <div class="page">
            <div class="container">
               <div class="row">
                  <!-- page with sidebar starts -->
                  <div class="col-lg-9 page-with-sidebar">
                     <div class="col-md-12">
                        <h2>Português 5º Ano</h2>
                        <!-- Image -->
                        <p class="h7">Para alunos de 10 e 11 anos.</p>
                        <p> Com conteúdo feito de acordo com a BNCC, nosso curso garante um aprendizado lúdico e significativo, 
						respeitando o ritmo e a individualidade de cada criança, para que ela realmente tenha prazer e alegria em aprender.</p>
                        <!-- row -->
                        <div class="row">
                           <div class="col-md-5">
                              <div class="col-md-12 carousel-1item owl-carousel owl-theme "  data-aos="fade-in" >
                                 <img src="../img/services/port5/PORT5-1.jpg" class="img-fluid rounded-circle" alt="">
                                 <img src="../img/services/port5/PORT5-2.jpg" class="img-fluid rounded-circle" alt="">
                                 <img src="../img/services/port5/PORT5-3.jpg" class="img-fluid rounded-circle" alt="">
                              </div>
                              <!-- /col-md- -->
                           </div>
                           <!-- /col-md -->
                           <div class="col-md-7">
                              <h4>Principais atividades</h4>
                              <!-- Image -->
                              <p>Para que a aula não se torne monótona ao aluno, os cursos foram criados utilizando o estilo de gamificação, onde o aluno não apenas realiza uma simples aula, 
                                 mas conta com desafios, conquistas, premiações, jogos, etc. 
                                 Tudo com uma linguagem totalmente apropriada para a faixa etária e um curso totalmente interativo para as crianças.  </p>
                              <!-- Button -->	 
                              <a href="https://www.dreduca.com.br/ead/Demonstrativos/PORT5/index.html" class="btn btn-secondary mt-2" target="_blank">Aula teste</a>
                              <a href="https://www.dreduca.com.br/ead/MATERIAL/port5/mobile/index.html" class="btn btn-primary mt-2" target="_blank">Material didático</a>
                           </div>
                           <!-- /col-md -->
                           <div class="col-md-12 mt-5">
                              <h4>Conteúdo Programático</h4>
                              <div class="accordion mt-3">
                                 <!-- collapsible accordion 1 -->
                                 <div class="card">
                                    <div class="card-header">
                                       <a class="collapsed card-link" data-toggle="collapse" href="#collapseOne">
                                       Módulo 1
                                       </a>
                                    </div>
                                    <!-- /card-header -->
                                    <div id="collapseOne" class="collapse" data-parent=".accordion">
                                       <div class="card-body">
                                          <ul class="custom pl-0">
                                             <li>Aula 1: Leitura e Interpretação de Texto.</li>
                                             <li>Aula 2: Tipos de Textos.</li>
                                             <li>Aula 3: Adjetivos.</li>
                                             <li>Aula 4: Adjetivos e Locução Adjetiva.</li>
                                             <li>Aula 5: Uso do X e Ch.</li>
                                             <li>Aula 6: Provérbios.</li>
                                             <li>Aula 7: Leitura e Interpretação de Texto.</li>
                                             <li>Aula 8: Uso do Mas e Mais.</li>
                                             <li>Aula 9: Preposição.</li>
                                             <li>Aula 10: Tipos de Textos: Informativos e Contos.</li>
                                             <li>Aula 11: Verbos.</li>
                                             <li>Aula 12: Pontuação.</li>
                                             <li>Aula 13: Uso da Letra L e U.</li>
                                             <li>Aula 14: Discurso Direto e Indireto.</li>
                                             <li>Aula 15: Uso da Letra M ou N.</li>
                                             <li>Aula 16: Revisão.</li>
                                              </ul>   </div>
                                    </div>
                                 </div>
                                 <!-- /card -->
                                 <!-- collapsible accordion 2 -->
                                 <div class="card">
                                    <div class="card-header">
                                       <a class="collapsed card-link" data-toggle="collapse" href="#collapseTwo">
                                       Módulo 2
                                       </a>
                                    </div>
                                    <div id="collapseTwo" class="collapse" data-parent=".accordion">
                                       <div class="card-body">
                                       <ul class="custom pl-0">
                                       <li> AULA 17 - Polissemia</li>
                                             <li> AULA 18 - Adivinhas, trava-línguas</li>
                                             <li> AULA 19 – Revisão</li>
                                             <li> AULA 20 - Interpretação de texto</li>
                                             <li> AULA 21 - Prefixo e sufixo</li>
                                             <li> AULA 22 - Tipos de textos poesia</li>
                                             <li> AULA 23 - Porque, por que, porquê e por quê</li>
                                             <li> AULA 24 - Leitura e interpretação de texto</li>
                                             <li> AULA 25 – Conjunção</li>
                                             <li> AULA 26 – Revisão</li>
                                             <li> AULA 27 - Uso do dicionário</li>
                                             <li> AULA 28 - Variedades linguísticas</li>
                                             <li> AULA 29 - Reescrita de histórias</li>
                                             <li> AULA 30 - Oxítonas, paroxítonas e proparoxítonas</li>
                                             <li> AULA 31 - Regras de jogos</li>
                                             <li> AULA 32 - Revisão</li>
                                       </ul>
                                       </div>
                                    </div>
                                 </div>
                                 <!-- /card -->
                                 <!-- collapsible accordion 3 -->
                                 <div class="card">
                                    <div class="card-header">
                                       <a class="collapsed card-link" data-toggle="collapse" href="#collapseThree">
                                       Módulo 3
                                       </a>
                                    </div>
                                    <div id="collapseThree" class="collapse" data-parent=".accordion">
                                       <div class="card-body">
                                       <ul class="custom pl-0">
                                             <li> AULA 33 - Formal e informal </li>
                                             <li> AULA 34 - Leitura e interpretação de texto</li>
                                             <li> AULA 35 - Tipo de texto diário</li>
                                             <li> AULA 36 -Seção, sessão</li>
                                             <li> AULA 37- Concordância entre pronomes e verbos</li>
                                             <li> AULA 38- Uso da vírgula, dois -pontos e ponto e vírgula</li>
                                             <li> AULA 39 – Revisão</li>
                                             <li> AULA 40 - Contos de terror</li>
                                             <li> AULA 41 - Leitura e interpretação de texto</li>
                                             <li> AULA 42- Parênteses, aspas e reticencias</li>
                                             <li> AULA 43 - Uso do travessão</li>
                                             <li> AULA 44 - Tipos de textos clássicos infantis</li>
                                             <li> AULA 45 – Revisão</li>
                                             <li> AULA 46 - Tipos de texto fábulas</li>
                                             <li> AULA 47 - Trás e traz</li>
                                             <li> AULA 48 - Leitura e interpretação de texto</li>
                                       </ul>
                                       </div>
                                    </div>
                                 </div>
                                 <!-- /card -->
                              </div>
                              <!-- /accordion -->
                           </div>
                           
                           <!-- /col-md-12-->
                           <div class="col-md-12 mt-5">
                           <?php include ("../cenarios/port5.php");?>
                           </div>
                           <!-- /col-md -->
                           <!-- custom link -->
                           <a class="custom-link float-right mt-5" href="../pages/index.php">Voltar para Home</a>
                        </div>
                        <!-- /row -->
                     </div>
                     <!-- /col-md-12 -->
                  </div>
                  <!-- /col-lg-9 -->
                  <!-- ==== Sidebar ==== -->
                  <div id="sidebar" class="col-lg-3 h-50 sticky-top">
                     <!--widget area-->
                     <div class="widget-area notepad">
                        <h5 class="sidebar-header">Informações</h5>
                        <div class="list-group">
                           <a href="#" class="list-group-item list-group-item-action active">
                           48 Horas
                           </a>
                           <a href="#" class="list-group-item list-group-item-action">Aulas interativas</a>
                           <a href="#" class="list-group-item list-group-item-action">Apostilas gamificadas</a>
                           <a href="#" class="list-group-item list-group-item-action">Exercícicos com jogos</a>
                           <a href="#" class="list-group-item list-group-item-action">Tecnologias inovadoras</a>
                        </div>
                        <!-- /list-group -->
                     </div>
                     <!-- /widget-area -->
                  </div>
                  <!-- /sidebar -->
               </div>
               <!-- /row-->
            </div>
            <!-- /container-->
         </div>
         <!-- /page -->
      </div>

<?php include ("../template/rodape.php");?>
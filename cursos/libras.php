<?php include ("../template/menu.php");?>
      <!-- /nav -->
      <!-- page wrapper starts -->
      <div id="page-wrapper">
         <!-- Jumbotron -->
         <div class="jumbotron jumbotron-fluid">
            <div class="container" >
               <!-- jumbo-heading -->
               <div class="jumbo-heading" data-aos="fade-down">
                  <h1>Cursos</h1>
                  <!-- Breadcrumbs -->
                  <nav aria-label="breadcrumb">
                     <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a>Home</a></li>
                        <li class="breadcrumb-item"><a>Cursos</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Português 1º Ano</li>
                     </ol>
                  </nav>
                  <!-- /breadcrumb -->
               </div>
               <!-- /jumbo-heading -->
            </div>
            <!-- /container -->
         </div>
         <!-- /jumbotron -->
         <!-- ==== Page Content ==== -->
         <div class="page">
            <div class="container">
               <div class="row">
                  <!-- page with sidebar starts -->
                  <div class="col-lg-9 page-with-sidebar">
                     <div class="col-md-12">
                        <h2>LIBRAS</h2>
                        <!-- Image -->
                        <p class="h7">Para alunos de 6 a 8 anos.</p>
                        <p> Este curso foi desenvolvido especialmente para oferecer ao aluno um ensino de forma dinâmica e divertida, 
                           com o objetivo de aprimorar e desenvolver ainda mais seu aprendizado e caso o aluno tenha dificuldade em alguma matéria este curso é perfeito.</p>
                        <!-- row -->
                        <div class="row">
                           <div class="col-md-5">
                              <div class="col-md-12 carousel-1item owl-carousel owl-theme "  data-aos="fade-in" >
                                 <img src="../img/services/libras/libras1.jpg" class="img-fluid rounded-circle" alt="">
                                 <img src="../img/services/libras/libras2.jpg" class="img-fluid rounded-circle" alt="">
                                 <img src="../img/services/libras/libras3.jpg" class="img-fluid rounded-circle" alt="">
                              </div>
                              <!-- /col-md- -->
                           </div>
                           <!-- /col-md -->
                           <div class="col-md-7">
                              <h4>Principais atividades</h4>
                              <!-- Image -->
                              <p>Para que a aula não se torne monótona ao aluno, os cursos foram criados utilizando o estilo de gamificação, onde o aluno não apenas realiza uma simples aula, 
                                 mas conta com desafios, conquistas, premiações, jogos, etc. 
                                 Tudo com uma linguagem totalmente apropriada para a faixa etária e um curso totalmente interativo para as crianças.  </p>
                              <!-- Button -->	 
                              <a href="https://www.dreduca.com.br/ead/Demonstrativos/LIBRAS/index.html" class="btn btn-secondary mt-2" target="_blank">Aula teste</a>
                              <a href="https://www.dreduca.com.br/ead/MATERIAL/libras1/mobile/index.html" class="btn btn-primary mt-2" target="_blank">Material didático</a>
                           </div>
                           <!-- /col-md -->
                           <div class="col-md-12 mt-5">
                              <h4>Conteúdo Programático</h4>
                              <div class="accordion mt-3">
                                 <!-- collapsible accordion 1 -->
                                 <div class="card">
                                    <div class="card-header">
                                       <a class="collapsed card-link" data-toggle="collapse" href="#collapseOne">
                                       Módulo 1 - Básico
                                       </a>
                                    </div>
                                    <!-- /card-header -->
                                    <div id="collapseOne" class="collapse" data-parent=".accordion">
                                       <div class="card-body">
                                          <ul class="custom pl-0">
                                             <li>Aula 1: Introdução à LIBRAS.</li>
                                             <li>Aula 2: Entrevista / Mercado de Trabalho / Salário.</li>
                                             <li>Aula 3: Numerais em LIBRAS.</li>
                                             <li>Aula 4: Entrevista em LIBRAS.</li>
                                             <li>Aula 5: Formas e Cores.</li>
                                             <li>Aula 6: Partes de Corpo e Características Físicas.</li>
                                             <li>Aula 7: Higiene Pessoal.</li>
                                             <li>Aula 8: Família e Sentimentos.</li>
                                             <li>Aula 9: Alimentos.</li>
                                             <li>Aula 10: Frutas e Legumes.</li>
                                             <li>Aula 11: Móveis e Cômodos de uma Casa.</li>
                                             <li>Aula 12: Vestuário.</li>
                                             <li>Aula 13: Semana, Mês e Estações do Ano.</li>
                                             <li>Aula 14: Eduação / Ensino.</li>
                                             <li>Aula 15: Profissões.</li>
                                             <li>Aula 16: Localidade.</li>
											            <li>Aula 17: Sistema Financeiro.</li>
											            <li>Aula 18: Natureza.</li>
                                              </ul>   </div>
                                    </div>
                                 </div>
                                 <!-- /card -->
                                 <!-- collapsible accordion 2 -->
                                 <div class="card">
                                    <div class="card-header">
                                       <a class="collapsed card-link" data-toggle="collapse" href="#collapseTwo">
                                       Módulo 2 - Intermediário
                                       </a>
                                    </div>
                                    <div id="collapseTwo" class="collapse" data-parent=".accordion">
                                       <div class="card-body">
                                       <ul class="custom pl-0">
                                       <li>Aula 1: Saudações.</li>
                                             <li>Aula 2: História do Surdo.</li>
                                             <li>Aula 3: Meios de Transporte.</li>
                                             <li>Aula 4: Legislação na Área da Surdez e Intérprete de LIBRAS.</li>
                                             <li>Aula 5: Meios de Comunicação.</li>
                                             <li>Aula 6: Parâmetros e Transcrição da LIBRAS.</li>
                                             <li>Aula 7: Órgãos Internos do Corpo Humano.</li>
                                             <li>Aula 8: Materiais de Construção.</li>
                                             <li>Aula 9: Sinais Religiosos.</li>
                                             <li>Aula 10: Medicamentos.</li>
                                             <li>Aula 11: Sinais Matemáticos.</li>
                                             <li>Aula 12: Português em LIBRAS.</li>
                                             <li>Aula 13: Pronomes Possessivos.</li>
                                             <li>Aula 14:Pronomes Indefinidos.</li>
                                             <li>Aula 15: Pronomes Demonstrativos / Interrogativos / Advérbio de Lugar.</li>
                                             <li>Aula 16: Verbo Ir e suas Variações.</li>
                                              </ul> 
                                       </div>
                                    </div>
                                 </div>
                                 <!-- /card -->
                                 <!-- collapsible accordion 3 -->
                                 <div class="card">
                                    <div class="card-header">
                                       <a class="collapsed card-link" data-toggle="collapse" href="#collapseThree">
                                       Módulo 3 - Avançado
                                       </a>
                                    </div>
                                    <div id="collapseThree" class="collapse" data-parent=".accordion">
                                       <div class="card-body">
                                       <ul class="custom pl-0">
                                       <li>Aula 1: Contação de História em LIBRAS.</li>
                                             <li>Aula 2: Poesia em LIBRAS.</li>
                                             <li>Aula 3: Profissionais de Saúde.</li>
                                             <li>Aula 4: Sexualidade.</li>
                                             <li>Aula 5: Técnicas de Tradução de Música.</li>
                                             <li>Aula 6: Contexto Jurídico.</li>
                                             <li>Aula 7: Sinais e seus Contextos.</li>
                                             <li>Aula 8: Estrutura Sintática.</li>
                                             <li>Aula 9: Informática.</li>
                                             <li>Aula 10: Classificadores.</li>
                                             <li>Aula 11: Sinais de Futebol.</li>
                                             <li>Aula 12: Localidade Espaciais Topográficas.</li>
                                             <li>Aula 13: Piadas.</li>
                                             <li>Aula 14: Gramática Formal e Informal.</li>
                                             <li>Aula 15: Esporte.</li>
                                             <li>Aula 16: Estados Brasileiros em LIBRAS.</li>

                                              </ul> 
                                       </div>
                                    </div>
                                 </div>
                                 <!-- /card -->
                              </div>
                              <!-- /accordion -->
                           </div>
                        
                           <!-- /col-md-12-->
                           <div class="col-md-12 mt-5">
                           <?php include ("../cenarios/libras.php");?>
                           </div>
                           <!-- /col-md -->
                           <!-- custom link -->
                           <a class="custom-link float-right mt-5" href="../pages/index.php">Voltar para Home</a>
                        </div>
                        <!-- /row -->
                     </div>
                     <!-- /col-md-12 -->
                  </div>
                  <!-- /col-lg-9 -->
                  <!-- ==== Sidebar ==== -->
                  <div id="sidebar" class="col-lg-3 h-50 sticky-top">
                     <!--widget area-->
                     <div class="widget-area notepad">
                        <h5 class="sidebar-header">Informações</h5>
                        <div class="list-group">
                           <a href="#" class="list-group-item list-group-item-action active">
                           48 Horas
                           </a>
                           <a href="#" class="list-group-item list-group-item-action">Aulas interativas</a>
                           <a href="#" class="list-group-item list-group-item-action">Apostilas gamificadas</a>
                           <a href="#" class="list-group-item list-group-item-action">Exercícicos com jogos</a>
                           <a href="#" class="list-group-item list-group-item-action">Tecnologias inovadoras</a>
                        </div>
                        <!-- /list-group -->
                     </div>
                     <!-- /widget-area -->
                  </div>
                  <!-- /sidebar -->
               </div>
               <!-- /row-->
            </div>
            <!-- /container-->
         </div>
         <!-- /page -->
      </div>

<?php include ("../template/rodape.php");?>